  ...
  <div class="min-h-screen bg-[#e9e9e9]">
    ...
    <main class="max-w-screen-2xl mx-auto px-4 py-12 sm:py-16 md:py-20">
      ...
      <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6 md:gap-8 mb-12">
        <div v-for="post in filteredPosts" :key="post.id" @click="goToBlog(post.id)" class="border border-black p-4 cursor-pointer hover:shadow-lg transition-shadow">
          <img :src="post.image" class="w-full h-48 sm:h-56 md:h-64 object-cover mb-4" />
          <div class="flex justify-between mb-3">
            <span class="font-poppins text-xs sm:text-sm text-gray-600">{{ post.category }}</span>
            <span class="font-poppins text-xs sm:text-sm text-gray-600">{{ post.author }}</span>
          </div>
          <h3 class="font-jetbrains font-bold text-lg sm:text-xl">{{ post.title }}</h3>
        </div>
      </div>

      <!-- More Button - Same size as category buttons -->
...
export default {
  ...
  data() {
    return {
      ...
      categories: ['ALL', 'MUSIC', 'ART', 'CULTURE', 'TECH', 'DESIGN', 'EVENT'],
      navItems: [
        { name: 'HOME', path: '/' },
      ...
      allPosts: [
        { id: 1, title: "The Evolution of Underground Music", category: "MUSIC", author: "John Doe", image: img1 },
        { id: 2, title: "Digital Art in Modern Era", category: "ART", author: "Jane Smith", image: img2 },
        { id: 3, title: "Cultural Preservation Through Art", category: "CULTURE", author: "Mike Johnson", image: img3 },
        { id: 4, title: "Annual Music Festival Recap", category: "EVENT", author: "Sarah Williams", image: img4 },
        { id: 5, title: "AI in Creative Industries", category: "TECH", author: "David Brown", image: img5 },
        { id: 6, title: "Minimalist Design Principles", category: "DESIGN", author: "Emma Wilson", image: img6 },
        { id: 7, title: "Jazz Fusion Revolution", category: "MUSIC", author: "Alex Turner", image: img1 },
        { id: 8, title: "Street Art Movement", category: "ART", author: "Lisa Wong", image: img2 },
        { id: 9, title: "Traditional Music Revival", category: "CULTURE", author: "Carlos Mendez", image: img3 }
      ],
      visiblePosts: 6
    }
  },
  computed: {
    filteredPosts() {
      ...
      if (this.selectedCategory !== 'ALL') {
        filtered = this.allPosts.filter(post => post.category === this.selectedCategory);
      }
